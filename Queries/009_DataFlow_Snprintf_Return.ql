...
where  call.getTarget().getName() = "snprintf" and 
	call.getArgument(2).getValue().regexpMatch("(?s).*%s.*") and
	source.asExpr() = call and
	sink.asExpr() = call.getArgument(1) and
	TaintTracking::localTaint(source, sink)

select call, "A call to snprintf with dataflow between return and the size argument stack variable as destination", call.getArgument(1)
